...
On start chee looks for a directory of name `.chee` in the current
working directory or above. If it exists it marks the directory as a
repository.

The repository is isolated from the global chee data. Chee will set
`chee.configdir` and `chee.workingdir` to the location containing the
`.chee` directory. The configuration file is read from
`.chee/chee.conf` and the working directory is `.chee/work/`. If a
config file doesn't exists, an empty one is created. All commands
executed are local to the repository directory.

By default only directories inside the repository can be added as
locations. But this check can be removed by setting
`chee.repo.restrict-to-root` to `false`.
